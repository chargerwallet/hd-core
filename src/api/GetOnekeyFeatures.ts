...
export default class GetChargerwalletatures extends BaseMethod {
  init() {
  ...
  async run() {
    const { message } = await this.device.commands.typedCall('ChargerwalletGetFeatures', 'Chargerwalletatures');
