...
export const processTxRequest = async ({
  ...
  let response: MessageResponse<'EthereumTxRequestChargerWallet'> | MessageResponse<'EthereumTxRequest'>;
  if (supportTrezor) {
    response = await typedCall('EthereumTxAck', 'EthereumTxRequest', {
      data_chunk: first,
    });
  } else {
    response = await typedCall('EthereumTxAckChargerWallet', 'EthereumTxRequestChargerWallet', {
      data_chunk: first,
    });
  }
...
export const evmSignTx = async ({
  ...
  if (supportTrezor) {
    ...
    response = await typedCall('EthereumSignTxChargerWallet', 'EthereumTxRequestChargerWallet', message);
  }
...
export const evmSignTxEip1559 = async ({
  ...
  if (supportTrezor) {
    ...
    response = await typedCall('EthereumSignTxEIP1559ChargerWallet', 'EthereumTxRequestChargerWallet', message);
  }
