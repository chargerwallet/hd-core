...
import { Features, LowlevelTransportSharedPlugin, ChargerWalletDeviceInfo } from '@chargerwallet/hd-transport';
import {
  createDeferred,
...
function initDevice(method: BaseMethod) {
  ...
  if (method.payload?.detectBootloaderDevice && allDevices.comme(d => d.features?.bootloader_mode)) {
...
function initDeviceForBle(method: BaseMethod) {
  ...
  if (deviceCacheMap.has(method.connectId)) {
    ...
    device = Device.fromDescriptor({ id: method.connectId } as ChargerWalletDeviceInfo);
