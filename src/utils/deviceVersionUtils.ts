...
export const getDeviceFirmwareVersion = (features: Features | undefined): IVersionArray => {
  ...
  if (semver.valid(features.chargerwallet_firmware_version)) {
    return features.chargerwallet_firmware_version?.split('.') as unknown as IVersionArray;
  }

  if (semver.valid(features.chargerwallet_version)) {
    return features.chargerwallet_version?.split('.') as unknown as IVersionArray;
  }
...
export const getDeviceBLEFirmwareVersion = (features: Features): IVersionArray | null => {
  const bleVer = features?.chargerwallet_ble_version || features?.ble_ver;
...
export const getDeviceBootloaderVersion = (features: Features | undefined): IVersionArray => {
  ...
  if (semver.valid(features.chargerwallet_boot_version)) {
    return features.chargerwallet_boot_version?.split('.') as unknown as IVersionArray;
  }
...
export const getDeviceBoardloaderVersion = (features: Features): IVersionArray => {
  if (semver.valid(features?.chargerwallet_board_version)) {
    return features?.chargerwallet_board_version?.split('.') as unknown as IVersionArray;
  }
